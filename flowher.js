...
import { poseTime } from './videotrack.js';

export class FlowHer {
  constructor(canvas, { sheet, meta, poses }) {
    this.cv = canvas; this.g = canvas.getContext('2d'); this.poses = poses;
    this.t = 0; this.gx = 0; this.gy = 0; this.from = null; this.mix = 1; this.ready = false;
  ...
  tick(dt, x, y, now) {
    ...
    const want = poseTime(this.poses, this.gx, this.gy, 1.5, this.t);
    if (Math.abs(want - this.t) > 0.9) { this.from = this.t; this.t = want; this.mix = 0; }   // the glance
    else this.t += (want - this.t) * (1 - Math.exp(-dt * 14));                                // the glide
  ...
  draw() {
    ...
    g.clearRect(0, 0, w, h);
    if (this.mix < 1 && this.from != null) { this.frame(this.from, 1); this.frame(this.t, this.mix); }
    else this.frame(this.t, 1);
  }

  // a moment on the take: the frame before it, and the frame after laid over it by how far between they are
  frame(t, alpha) {
    const { fps, n } = this.m, f = Math.min(n - 1, Math.max(0, t * fps)), i = Math.floor(f), u = f - i;
    this.cell(i, alpha);
    if (u > 0.02 && i + 1 < n) this.cell(i + 1, alpha * u);
  }
  cell(i, alpha) {
