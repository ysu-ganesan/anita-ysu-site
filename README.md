...
6. **Later on 25 Sep: the glitch, and her eyes on the left.**
   ...
   - **She notices you, then turns.** When the cursor appears while she's mid-sway, her resting motion plays 3× faster, forward or back, whichever is nearer, into the nearest idle frame where her head matches the look pose and her lean is within 3.5 px (`lookFrom` in `idle.json`). That takes at most 0.32 s. Then she turns with a 0.18 s blend. Before, she waited up to 0.6 s, blended in from any pose, and slid back upright from as much as 19 px of lean.
